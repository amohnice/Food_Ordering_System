...
function display_notif(type, info = "No info provided") {
    ...
    switch (type) {
        case "add to cart":
            toastr.success(info.item_description + ': Ksh ' + info.price, 'Added to Cart');
            break;
...
function load_cart() {
    ...
    if (cart !== null && cart.length > 0) {
        for (var i = 0; i < cart.length; i++) {
            ...
            item_price.innerHTML = "Ksh " + parseFloat(cart[i].price).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 });

            total += cart[i].price
        }
        total = Math.round(total * 100) / 100
        localStorage.setItem('total_price', total);
        document.getElementById('total').innerHTML = "Ksh " + parseFloat(localStorage.getItem("total_price")).toLocaleString('en-US', { style: 'decimal', maximumFractionDigits: 2, minimumFractionDigits: 2 })
